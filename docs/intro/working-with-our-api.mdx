...
First of all, you need to **sign up** before you can start consuming the API. You can sign up [here](https://admin.goschedule.io/login).  
You also need to create a **GoSchedule app**, as all requests to the API are made in the context of an app.
...
GoSchedule uses **JSON** as its data format, so all data to and from GoSchedule must be and will be formatted in JSON.  
...
```bash
curl --request GET \
  --url https://api.goschedule.io/v2/bookings \
